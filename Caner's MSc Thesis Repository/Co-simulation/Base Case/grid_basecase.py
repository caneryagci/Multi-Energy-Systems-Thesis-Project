...
def grid_net():
    ...
    #pp.create_transformer_from_parameters(net, bus1, bus2, sn_mva=20, vn_hv_kv=110, vn_lv_kv=33, vkr_percent=1, vk_percent=1, pfe_kw=1, i0_percent=1, shift_degree=0, tap_phase_shifter=False, in_service=True, name='Trafo', index=None, max_loading_percent=90, parallel=1, df=1.0)
        ...
        #create 110 kV lines
    pp.create_line(net, bus2, bus3, length_km=20., std_type='NAYY 4x120 SE', max_loading_percent=90)
        
        #create loads
    l1 = pp.create_load(net, bus3, p_mw=50, q_mvar=0, sn_mva=50, min_p_mw=Pmin_p2g,max_p_mw=Pmax_p2g, max_q_mvar=10, min_q_mvar=-10, scaling=1.0, in_service=True, controllable=False,name='Power2Gas',)
    ...
    l3 = pp.create_load(net, bus3, p_mw=20, q_mvar=0, sn_mva=20, min_p_mw=0,max_p_mw=20, max_q_mvar=20, min_q_mvar=-20, scaling=1.0, in_service=True, controllable=False,name='Electrical')
        
        #create generators
    eg = pp.create_ext_grid(net, bus1,vm_pu=1.0, va_degree=0,min_p_mw=Pmin_extgrid, max_p_mw=Pmax_extgrid, name='external_grid')
    g0 = pp.create_sgen (net, bus3, p_mw=80, q_mvar=0,   sn_mva=100, name='WF', max_q_mvar=80, min_q_mvar=-80, min_p_mw=0, max_p_mw=100, scaling=1.0, type=None, controllable=False, in_service=True)
    g1 = pp.create_sgen(net, bus3, p_mw=60,  q_mvar=0,  sn_mva=100, name='PV', max_q_mvar=80, min_q_mvar=-80, min_p_mw=0, max_p_mw=100, scaling=1.0, type=None,  controllable=False, in_service=True)
    
    pp.runpp(net, calculate_voltage_angles=False, verbose=True)
    return net
...
# save as pickle
pp.to_pickle(net, os.path.join(simulators_dir, 'gridModel_basecase.p'))  # absolute path
