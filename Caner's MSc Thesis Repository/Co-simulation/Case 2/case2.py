...
p2h_loc = os.path.join(simulators_dir, 'P2H.pth_modelB_case2.fmu')
p2g_loc = os.path.join(simulators_dir, 'Hydrogen.ptg_modelB_case2.fmu')
grid_loc = os.path.join(simulators_dir, 'gridModel_case2.p')


my_world.add_powerflow('grid1', grid_loc, inputs = ['WF.p_mw','PV.p_mw', 'Power2Gas.max_p_mw','Power2Gas.cp1_eur_per_mw','Power2Heat.cp1_eur_per_mw','Power2Gas.min_p_mw','Power2Heat.max_p_mw','Power2Heat.min_p_mw'], 
                                                    outputs=['Bus 0.vm_pu', 'Bus 1.vm_pu', 'Bus 2.vm_pu', 'WF.p_mw', 'PV.p_mw',
                                                             ...
                                                             'WF.max_p_mw','PV.max_p_mw'], step_size=300)

my_world.add_fmu(fmu_name = 'p2g', fmu_loc = p2g_loc, step_size = 1, inputs = ['gas_demand','P_order','T_ambient','ptg_switch'], 
                 outputs=[ 'electrolyser_detailed1.Pelec','lcoh.cost','gas_demand','electrolyser_detailed1.electrochemical.efficiency2','storage2.S_storage','controller_P2G3.Pmin','controller_P2G3.Pmax','electrolyser_detailed1.nH2','P_order','ptg_switch'], variable=False, validate=False)

my_world.add_fmu(fmu_name = 'p2h', fmu_loc = p2h_loc, step_size = 1, inputs = ['heat_demand','T_ambient','P_order','pth_switch'], 
...
df.to_csv(r'C:\Users\Caner\Desktop\Multi-Energy-Systems-Thesis-Project\Co_simulation\Case 2\Flexibledemand.csv')

t = res['p2g'].time
hydrogen_prod = res['p2g'].loc[:,'p2g.electrolyser_detailed1.nH2']
...
df1.to_csv(r'C:\Users\Caner\Desktop\Multi-Energy-Systems-Thesis-Project\Co_simulation\Case 2\hydrogen_prod.csv')
...
df2.to_csv(r'C:\Users\Caner\Desktop\Multi-Energy-Systems-Thesis-Project\Co_simulation\Case 2\heat_prod.csv')
...
ax22.plot(time, ptg_cost, 'blue', linewidth=1, label = 'PtG cost')
ax22.plot(time, pth_cost, 'red', linewidth=1, label = 'PtH cost')
ax22.set_ylabel("Cost [EUR/MWh]", fontsize=15)
ax22.set_xlabel("Time [h]", fontsize='large')
ax22.legend(fontsize=15)
#plt.show()
ax22.set_xticks(seconds) 
ax22.grid('on')
ax22.set_xticklabels(hours)
...
df3 = pd.DataFrame (data3, columns = ['time','ptg_cost','pth_cost'])
df3.to_csv(r'C:\Users\Caner\Desktop\Multi-Energy-Systems-Thesis-Project\Co_simulation\Case 2\cost.csv')
...
ax1.plot(time, electrolyser_load, 'blue', linewidth=1, label='P2G(MW)')
ax1.plot(time, heatpump_load, 'red', linewidth=1, label='P2H(MW)')
...
ax1.set_xticks(seconds) 
ax1.grid('on')
ax1.set_xticklabels(hours)
plt.show()
plt.savefig('MES_P.png')
plt.savefig('MES_P.pdf',bbox_inches='tight', pad_inches=0.01)
data4 = {'time':  time,
        'ptg_load': electrolyser_load,
        'pth_load': heatpump_load,
        'excess_RE': electrical_load
        }
df4 = pd.DataFrame (data4, columns = ['time','ptg_load','pth_load','excess_RE'])
df4.to_csv(r'C:\Users\Caner\Desktop\Multi-Energy-Systems-Thesis-Project\Co_simulation\Case 2\Load_P.csv')
...
ax21.set_ylabel("Active Power [MW]", fontsize=15)
ax21.set_xlabel("Time [s]", fontsize=15)
ax21.legend(fontsize=15)
#plt.show()
plt.savefig('Pminmax.png')
plt.savefig('Pminmax.pdf',bbox_inches='tight', pad_inches=0.01)
