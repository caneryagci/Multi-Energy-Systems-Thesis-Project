...
#cost = -100
my_world = World(stop_time=21600, logging = True, clean_up = False, interpolate_results=False, exchange = 900)

simulators_dir = r'C:\Users\Caner\Desktop\Multi-Energy-Systems-Thesis-Project\Co_simulation\Case 2'
p2h_loc = os.path.join(simulators_dir, 'P2H.pth_modelB_case1.fmu')
p2g_loc = os.path.join(simulators_dir, 'Hydrogen.ptg_modelB_case1.fmu')
grid_loc = os.path.join(simulators_dir, 'gridModel_case2.p')


my_world.add_powerflow('grid1', grid_loc, inputs = ['WF.p_mw', 'PV.p_mw', 'Power2Heat.p_mw','Power2Gas.p_mw','Power2Gas.cp1_eur_per_mw','Power2Heat.cp1_eur_per_mw'], outputs=['Bus 0.vm_pu', 'Bus 1.vm_pu', 'Bus 2.vm_pu', 'WF.p_mw', 'PV.p_mw',
                       'Electrical.p_mw', 'Power2Heat.p_mw', 'Power2Gas.p_mw','grid.p_mw','grid.q_mvar','Electrical.q_mvar','Power2Gas.cp1_eur_per_mw','Power2Heat.cp1_eur_per_mw'], step_size=300)


my_world.add_fmu(fmu_name = 'p2g', fmu_loc = p2g_loc, step_size = 3, inputs = ['gas_demand','T_ambient','P_order'], 
                 outputs=[ 'electrolyser_detailed1.Pelec','lcoh.cost','gas_demand','electrolyser_detailed1.electrochemical.efficiency2','storage2.S_storage','controller_P2G3.Pmin','controller_P2G3.Pmax','P_order'], variable=False, validate=False)

my_world.add_fmu(fmu_name = 'p2h', fmu_loc = p2h_loc, step_size = 3, inputs = ['heat_demand','T_ambient','P_order'], 
                 outputs=[ 'hp2.Pelec','lcodh.cost','heat_demand','hp2.COP','storage.S','controller_APL.Pmin','controller_APL.Pmax','P_order'], variable=False, validate=False)
