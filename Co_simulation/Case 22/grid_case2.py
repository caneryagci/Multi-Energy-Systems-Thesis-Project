...
def grid_net():
    ...
    pp.create_line(net, bus2, bus3, length_km=10., std_type='NAYY 4x120 SE', max_loading_percent=100)
        ...
        #create loads
    l1 = pp.create_load(net, bus3, p_mw=50, q_mvar=0, sn_mva=55, min_p_mw=10,max_p_mw=50, max_q_mvar=10, min_q_mvar=-10, scaling=1.0, in_service=True, controllable=True,name='Power2Gas',)
    l2 = pp.create_load(net, bus3, p_mw=50, q_mvar=0, sn_mva=55, min_p_mw=20,max_p_mw=48, max_q_mvar=10, min_q_mvar=-10, scaling=1.0, in_service=True, controllable=True,name='Power2Heat',)
    ...
    l4 = pp.create_load(net, bus3, p_mw=1, q_mvar=0, sn_mva=20, min_p_mw=0,max_p_mw=20, max_q_mvar=20, min_q_mvar=-20, scaling=1.0, in_service=True, controllable=False,name='Electrical_base')
            
        #create generators
    eg = pp.create_ext_grid(net, bus1,vm_pu=1.0, va_degree=0,min_p_mw=Pmin_extgrid, max_p_mw=Pmax_extgrid, name='external_grid')
    g0 = pp.create_sgen (net, bus3, p_mw=88, q_mvar=0,   sn_mva=110, name='WF', max_q_mvar=80, min_q_mvar=-80, min_p_mw=0, max_p_mw=Pmax_WF, scaling=1.0, type=None, controllable=True, in_service=True)
    g1 = pp.create_sgen(net, bus3, p_mw=0,  q_mvar=0,  sn_mva=110, name='PV', max_q_mvar=80, min_q_mvar=-80, min_p_mw=0, max_p_mw=Pmax_PV, scaling=1.0, type=None,  controllable=True, in_service=True)
