...
#cost = -100
my_world = World(stop_time=21600, logging = True, clean_up = False, interpolate_results=False, exchange = 300)
...
my_world.add_powerflow('grid1', grid_loc, inputs = ['WF.P', 'PV.P','Power2Heat.P'], outputs=['Bus 0.V', 'Bus 1.V', 'Bus 2.V', 'WF.P', 'PV.P',
                       'Electrical.P', 'Power2Heat.P', 'Power2Gas.P','grid.P','grid.Q','Electrical.Q' ], step_size=100)


my_world.add_fmu(fmu_name = 'p2g', fmu_loc = p2g_loc, step_size = 3, inputs = ['gas_demand','T_ambient','Porder'], 
...
my_world.add_csv('data', os.path.join(simulators_dir, 'flexdata.csv'))
...
connections = {#'cprice.y':'controller.C',  #data to fmu
               ...
               'grid1.Power2Gas.P':'p2g.electrolyser_detailed1.Porder',
